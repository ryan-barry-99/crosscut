...
- `crosscut present [--vs <rev> | --rebase <rev> | --uncommitted | --last <n> | --branch] [--ref <branch>] [--file <path>]` opens the current worktree's changes (or a branch's) in the VS Code window showing the repo, as one multi-file diff. A comparison flag that differs from the row's opens the diff as its own row under *Opened commits & PRs*, leaving the row you were looking at alone. Meant for agents: run it from a terminal to show someone a change against the right base. If no window shows the repo, the window focused last adds it to its tree. In a folder with no git repo, Crosscut snapshots the folder into a history of its own under `~/.cache/crosscut/shadow` and shows what changed since the last present.
...
- `crosscut present --pr <n>` opens that pull request's row under **Open pull requests**, with its review comments inline, your own pending (unsubmitted) review included and marked as such.
- Any `present` form can point at files. A spec is a path (a folder covers everything under it), optionally with lines on the new side: `path:12` or `path:12-30`. `--mark <spec>...` highlights those lines in the full diff, `--only <spec>...` shows just those files with their lines highlighted, `--open <spec>` opens one file in its own side-by-side diff at those lines, and `--file <spec>` scrolls the full view to it. The multi-file view scrolls to a file, not a line, so `--only` with a single file and lines opens the side-by-side diff instead. Presenting review findings is one command: `crosscut present --pr 381 --mark src/a.ts:40-52 src/b.ts:7`.
- `crosscut link <present arguments> [--text <label>] [--chat]` prints a link that does what that `present` would when clicked, in the window you click it in (a repo that window does not show joins its tree). Everything is resolved when the link is made, so it keeps pointing at the same commits. The plain form is a `vscode://` link, for the terminal or a markdown file; `--text` prints it as a markdown link. `--chat` links to a small file under `~/.cache/crosscut/links/` instead, for chat panels (the Claude Code one included) that open file links but drop `vscode://` ones: the extension runs the file when it opens and closes its tab. That is how an agent gives you a table of findings you can click through.
