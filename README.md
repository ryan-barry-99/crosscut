...
- `crosscut present [--vs <rev> | --rebase <rev> | --uncommitted | --last <n> | --branch] [--ref <branch>] [--file <path>]` opens the current worktree's changes (or a branch's) in the VS Code window showing the repo, as one multi-file diff. A comparison flag that differs from the row's opens the diff as its own row under *Opened commits & PRs*, leaving the row you were looking at alone. Meant for agents: run it from a terminal to show someone a change against the right base.
